...
public class CSVImpoterEditor : Editor
{
  ...
  void SetCsvDataToScriptableObject(CSVImporter csvImporter)
  {
    ...
    for (int i = 1; i < afterParse.Length; i++)
    {
      ...
      var antData = CreateInstance<AntStats_old>();
      ...
      // タイプ
      column += 1;
      var typeName = Enum.Parse<UnitType_old>(parseByComma[column]);
      ...
      var asset = (AntStats_old)AssetDatabase.LoadAssetAtPath(path, typeof(AntStats_old));
      if (asset == null)
      {
